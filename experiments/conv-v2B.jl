...
function _point_conv_lvB!(y, x, w)    
    _dot_prod_lvB!(y, x, w)
    return y
end
...
_point_conv_lvB!(yi, w_flat, xi_flat)
# @btime _point_conv_lvB!($yi, $w_flat, $xi_flat);
# @time yi_out = _point_conv_lv!(yi, xi, w_flat);
# @btime yi_out = _point_conv_lv!(yi, xi, w_flat);

function my_conv_3B!(y, x, w)
    y .= 0
...
@time my_conv_3B!(y, x, w);
# 834.700 μs (50339 allocations: 2.33 MiB)
@btime my_conv_3B!($y, $x, $w);
std(y)
@code_warntype my_conv_3B!(y, x, w);
...
@time y3 = my_conv_3B(x, w);
# 1.014 ms (50343 allocations: 4.09 MiB)
@btime my_conv_3B($x, $w);
std(y3)

@code_warntype my_conv_3B(x, w);

using Enzyme
loss(x, w) = sum(my_conv_3B(x, w))
dw = zero(w);
@time loss(x, w)
